...
export interface StorageGroup {
  /** Auto-incrementing surrogate id, generated by the DB (absent until persisted) */
  id?: number;
  name: string;
  // is this group is in use
  active: boolean;
}
...
export interface ServerStorageProvider {
   ...
   * the user's `id`. Existing rows have their `role` refreshed.
   */
  upsertUser(email: string, groups: string[], role: 'admin' | 'user'): Promise<string>;
  /** Look up a user by email; returns the user's `id`, `email`, and parsed `data` blob, or `null` if not found */
  userExists(
    email: string
  ): Promise<string | null>;
  /**
  ...
  /** List the access groups a user belongs to (via `user_group_mapping`) */
  getGroupsByUserId(userId: string): Promise<StorageGroup[]>;
  /**
   * Upsert access groups into the `user_group` table; returns the id of each
   * group in input order, whether it was newly created or already existed.
   */
  createGroups(groups: StorageGroup[]): Promise<number[]>;
  /**
   ...
   */
  mapUserToGroup(userId: string, groupIds: number[]): Promise<void>;
